...
/**
 * 动态光照调度器(26.2 空间查找 + 跨帧光源跟踪)。
 * <p>
 * 双路径:
 * - 方块路径:{@code LightCoordsUtilMixin} 注入 {@code BrightnessGetter.DEFAULT}(lambda$static$0),
 *   借助 BlockModelLighter.Cache 的 LRU(每方块每编译最多一次查询),GPU 顶点插值实现跨方块平滑
 ...
 * 命中且相同(静止)→ 跳过;旧缓存未被命中的(移除)→ 刷新。区块编译频率不做限制。
 * </p>
 * <p>
 * 精度:提升时写入 {@code (int)(level * 16.0)} 到 light coords 的 block 位
 * (Lamb 同款位运算)——4-bit 读取时等效 floor 安全,smooth(8-bit)读取时保留 16 倍精度。
 * </p>
 */
public final class DynamicLightDispatcher {
    ...
    /**
     * 帧末调度:构建空间查找快照并跨帧对比光源,重编译位置变化/新增/移除的光源影响区块。
     * <p>
     * 26.2: 区块重编译入口为 {@link net.minecraft.client.renderer.extract.LevelExtractor#setSectionDirty(int, int, int)}。
        ...
        if (LightSources.isEmpty()) {
            ...
            if (old.sources.length != 0) {
                ...
                for (long sec : sections) {
                    extractor.setSectionDirty(SectionPos.x(sec), SectionPos.y(sec), SectionPos.z(sec));
                }
    ...
    /**
     * 方块路径(26.2):由 {@code LightCoordsUtilMixin} 在 {@code BrightnessGetter.DEFAULT} 调用,
     * 返回提升后的 light coords。借助 vanilla LRU 缓存,每方块每编译最多一次查询。
     */
    public static int getDynamicLight(BlockAndLightGetter level, BlockState state, BlockPos blockPos, int originalLight) {
        ...
        int y = blockPos.getY();
        int z = blockPos.getZ();
        double dynamicLight = getDynamicLightLevel(x + 0.5, y + 0.5, z + 0.5);
        if (dynamicLight > 0) {
     ...
     * 用实体眼睛的<b>连续世界坐标</b>精确计算光照贡献(不经 floor 方块化,
     * 光照随实体移动连续平滑变化,跨方块边界无阶跃跳变)。
     *
     * @param eyePos       实体眼睛的连续世界坐标
     * @param originalLight vanilla 原始 light coords
     * @return 提升后的 light coords(仅 block 位可能提升,sky 位保持不变)
     */
    ...
    /**
     * 查询精确位置 (x, y, z) 的动态光照(连续坐标,直接与光源位置求距离)。
     * <p>
    ...
    /**
     * 将动态光照写入 light coords 的 block 位。
     * 位运算(Lamb 同款):(level * 16) 写入低 20 位,4-bit 读取时等效 floor 安全,
     * smooth(8-bit)读取时保留 16 倍精度,边缘过渡平滑。
     */
